  ...
  module Testing
    class DOMTest < Picotest::Test
      ...
      def assert_text(expected, selector = nil)
        assert(text(selector).include?(expected.to_s))
      end
...
function summarize(results) {
  ...
  for (const [name, result] of Object.entries(results)) {
    ...
    for (const failure of result.failures || []) {
      console.log(`  F ${failure.method}: ${failure.error_message}`);
    ...
    for (const exception of result.exceptions || []) {
      console.log(`  E ${exception.method}: ${exception.raise_message}`);
    }
  }
